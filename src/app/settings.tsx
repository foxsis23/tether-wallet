...
import { getCurrentWalletMeta, removeWallet } from '@/services/wallet-store';
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              // Remove from WDK provider first
              await clearWallet();
              await clearAvatar();

              // Remove from local wallet store
              const meta = await getCurrentWalletMeta();
              if (meta?.id) {
                await removeWallet(meta.id);
              }
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoCard}>
            <TouchableOpacity
              style={styles.manageWalletsButton}
              onPress={() => router.push('/wallets')}
              activeOpacity={0.7}
            >
              <Text style={styles.manageWalletsText}>View and switch wallets</Text>
            </TouchableOpacity>
          </View>
        </View>
